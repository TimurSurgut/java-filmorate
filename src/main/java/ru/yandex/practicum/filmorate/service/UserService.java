...
public class UserService {
    ...
    public ResponseEntity<?> addFriend(int id, int idFriend) {
        if (!userStorage.findAll().containsKey(id)) {
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + id + " не зарегистрирован"), HttpStatus.NOT_FOUND);
        } else if (userStorage.findAll().containsKey(idFriend)) {
            if (userStorage.findAll().get(id).getFriends().contains(idFriend)) {
                return new ResponseEntity<>(gson.toJson("Пользователь с id " + idFriend + " уже добавлен в друзья"), HttpStatus.OK);
            } else {
                userStorage.findAll().get(id).getFriends().add(idFriend);
                userStorage.findAll().get(idFriend).getFriends().add(id);
            }
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + idFriend + " успешно добавлен в друзья"), HttpStatus.OK);
        } else {
            return new ResponseEntity<>(gson.toJson("Невозможно добавить в друзья не зарегистрированного пользователя"), HttpStatus.NOT_FOUND);
        }
    }
    ...
    public ResponseEntity<?> deleteFriend(int id, int idFriend) {
        if (!userStorage.findAll().containsKey(id)) {
            ...
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + idFriend + " не зарегистрирован"), HttpStatus.NOT_FOUND);
        } else if (!userStorage.findAll().get(id).getFriends().contains(id)) {
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + idFriend + " отсутствует в списке друзей"), HttpStatus.OK);
        } else {
            userStorage.findAll().get(id).getFriends().remove(idFriend);
            userStorage.findAll().get(idFriend).getFriends().remove(id);
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + idFriend + " успешно удалён из друзей"), HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<?> getAllFriends(int id) {
        if (!userStorage.findAll().containsKey(id)) {
            ...
            return new ResponseEntity<>(gson.toJson("У пользователя с id " + id + " список друзей пуст"), HttpStatus.NOT_FOUND);
        } else {
    ...
    public ResponseEntity<?> getCommonFriends(int id, int otherId) {
        ...
        if (!userStorage.findAll().containsKey(id)) {
            ...
            return new ResponseEntity<>(gson.toJson("Пользователь с id " + otherId + " не зарегистрирован"), HttpStatus.NOT_FOUND);
